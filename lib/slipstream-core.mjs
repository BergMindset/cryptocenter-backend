...
export const DEFAULTS = {
  ...
  maxTickSpacing: 10, // K10: пулы с крупным шагом тиков пока не поддерживаем (округление врёт)
}

// K2: белый список USD-стейблов на Base. Пара считается «стейбл» только если ОБА токена здесь
// И цена ≈1 — иначе не-долларовая пара с курсом ~1 (ezETH/WETH) прошла бы фильтр и увела бюджет в космос.
export const BASE_STABLES = new Set(
  [
...
// K1/K3: кэш верифицированной меты пула. Мета пула неизменна (токены/decimals/фабрика/NFPM),
// поэтому храним навсегда; tickTTL — короткий срез цены. Живёт в памяти процесса.
const metaCache = new Map() // poolAddrLower -> PoolMeta (без tick/price — те свежие)
export function getCachedMeta(poolAddr) {
  return metaCache.get(String(poolAddr).toLowerCase()) ?? null
}
...
export async function verifyPool(client, poolAddr) {
  ...
  // K3: кэшируем неизменную часть меты (токены/decimals/tickSpacing) — poolTick берёт decimals отсюда,
  // а не хардкодит 6/6 (иначе цена пары с DAI наврала бы в 10^12).
  metaCache.set(String(poolAddr).toLowerCase(), meta)
  return meta
}

/** Волатильность пары: MVP — только стейбл-пары. K2: ОБА токена из белого списка USD-стейблов
 *  И |цена − 1| ≤ 5%. Иначе не-долларовая пара с курсом ~1 (ezETH/WETH) увела бы бюджет в космос. */
export function assertStablePair(meta) {
  const t0 = BASE_STABLES.has(String(meta.token0.addr).toLowerCase())
  const t1 = BASE_STABLES.has(String(meta.token1.addr).toLowerCase())
  if (!t0 || !t1)
    throw new Error(`пара ${meta.token0.symbol}/${meta.token1.symbol} не из списка USD-стейблов Base — этап 2 (нужен ценовой источник)`)
  if (Math.abs(meta.price - 1) > 0.05)
    throw new Error(`пара ${meta.token0.symbol}/${meta.token1.symbol} волатильная (цена ${meta.price.toFixed(4)}) — вход не строим`)
  if (meta.tickSpacing > DEFAULTS.maxTickSpacing)
    throw new Error(`шаг тиков пула ${meta.tickSpacing} > ${DEFAULTS.maxTickSpacing} — округление коридоров ненадёжно, вход не строим`)
}
...
export function buildEnterPlan(meta, budgetUsd, owner, opts = {}) {
  ...
  // K10: сначала округляем тики по спейсингу, ЗАТЕМ считаем суммы от ФАКТИЧЕСКИХ границ цены
  // округлённых тиков (а не от «красивых» ±%). Иначе на крупном шаге тиков суммы не совпадут
  // с реальным диапазоном mint — часть капитала не зайдёт или транзакция тихо провалится.
  const tickToPrice = (tick) => Math.pow(1.0001, tick) * 10 ** (meta.token0.decimals - meta.token1.decimals)
  const plans = corridors.map((c) => {
    ...
    const { amount0, amount1 } = corridorAmounts(meta, tickToPrice(tickLower), tickToPrice(tickUpper), c.capitalUsd)
...
const NFPM_OWNER_ABI = parseAbi(['function ownerOf(uint256) view returns (address)'])
const ERC20_BAL_ABI = parseAbi(['function balanceOf(address) view returns (uint256)'])

/** B7: хватает ли у кошелька обоих токенов на план входа. Возвращает {ok, need} для честного «не хватает N». */
export async function checkBalances(client, meta, owner, total0, total1) {
  const [b0, b1] = await Promise.all([
    client.readContract({ address: meta.token0.addr, abi: ERC20_BAL_ABI, functionName: 'balanceOf', args: [owner] }),
    client.readContract({ address: meta.token1.addr, abi: ERC20_BAL_ABI, functionName: 'balanceOf', args: [owner] }),
  ])
  const short0 = b0 < total0
  const short1 = b1 < total1
  const h = (v, d) => Number(v) / 10 ** d
  return {
    ok: !short0 && !short1,
    have: { [meta.token0.symbol]: h(b0, meta.token0.decimals), [meta.token1.symbol]: h(b1, meta.token1.decimals) },
    short: [
      short0 ? { sym: meta.token0.symbol, need: h(total0 - b0, meta.token0.decimals) } : null,
      short1 ? { sym: meta.token1.symbol, need: h(total1 - b1, meta.token1.decimals) } : null,
    ].filter(Boolean),
  }
}

/** K9: подтвердить, что позиция принадлежит owner, и взять ЖИВУЮ ликвидность + текущий тик пула
 *  из чейна (не из запроса клиента). Возвращает всё нужное для расчёта защиты выхода (B3). */
export async function verifyPosition(client, tokenId, owner) {
  if (!/^\d+$/.test(String(tokenId))) throw new Error('tokenId должен быть целым')
  if (!/^0x[0-9a-fA-F]{40}$/.test(owner)) throw new Error('некорректный адрес кошелька')
  const id = BigInt(tokenId)
  const [onchainOwner, pos] = await Promise.all([
    client.readContract({ address: AERODROME.nfpm, abi: NFPM_OWNER_ABI, functionName: 'ownerOf', args: [id] }),
    client.readContract({ address: AERODROME.nfpm, abi: NFPM_ABI, functionName: 'positions', args: [id] }),
  ])
  if (String(onchainOwner).toLowerCase() !== owner.toLowerCase())
    throw new Error(`позиция #${tokenId} принадлежит не вам`)
  const [token0, token1, tickSpacing, tickLower, tickUpper, liquidity] = [pos[2], pos[3], Number(pos[4]), Number(pos[5]), Number(pos[6]), pos[7]]
  if (liquidity === 0n) throw new Error(`позиция #${tokenId} уже пуста`)
  // Текущий тик пула + decimals токенов — для расчёта amount0Min/amount1Min (B3).
  ...
  return { tokenId: id, liquidity, tickLower, tickUpper, tickCurrent: Number(slot0[1]), decimals0: Number(dec0), decimals1: Number(dec1) }
}

/** План выхода одной NFT-позиции юзера: снять ликвидность → забрать токены → сжечь NFT.
 *  B3: amount0Min/amount1Min считаем от ожидаемых сумм минус запас (по умолчанию 5%) —
 *  защита от сдвига цены во время выхода; при этом выход всегда должен проходить (не «заложник»). */
export function buildExitTxs(pos, owner, opts = {}) {
  const deadline = BigInt(Math.floor(Date.now() / 1000) + (opts.deadlineMinutes ?? DEFAULTS.deadlineMinutes) * 60)
  const slip = opts.exitSlippagePct ?? 5
  let amount0Min = 0n
  let amount1Min = 0n
  if (pos.tickCurrent != null && pos.decimals0 != null) {
    const { a0, a1 } = amountsForLiquidity(Number(pos.liquidity), pos.tickCurrent, pos.tickLower, pos.tickUpper)
    const f = BigInt(Math.floor((1 - slip / 100) * 1000))
    amount0Min = (BigInt(Math.floor(a0)) * f) / 1000n
    amount1Min = (BigInt(Math.floor(a1)) * f) / 1000n
  }
  const mk = (step, data, gas) => ({ step, to: AERODROME.nfpm, data, value: '0', gasLimitHint: gas })
  return [
    mk('decreaseLiquidity', encodeFunctionData({ abi: NFPM_ABI, functionName: 'decreaseLiquidity', args: [{ tokenId: pos.tokenId, liquidity: pos.liquidity, amount0Min, amount1Min, deadline }] }), '300000'),
    mk('collect', encodeFunctionData({ abi: NFPM_ABI, functionName: 'collect', args: [{ tokenId: pos.tokenId, recipient: owner, amount0Max: MAX_UINT128, amount1Max: MAX_UINT128 }] }), '200000'),
    mk('burn', encodeFunctionData({ abi: NFPM_ABI, functionName: 'burn', args: [pos.tokenId] }), '150000'),
  ]
}

/** Лёгкий живой срез пула: тик/цена/ликвидность (для секундных обновлений на фронте).
 *  K3: decimals берём из кэша верификации — без верификации тик не отдаём (иначе цена наврёт). */
export async function poolTick(client, poolAddr) {
  if (!/^0x[0-9a-fA-F]{40}$/.test(poolAddr)) throw new Error('некорректный адрес пула')
  const cached = getCachedMeta(poolAddr)
  if (!cached) throw new Error('пул не верифицирован — сначала /verify')
  const { decimals: dec0 } = cached.token0
  const { decimals: dec1 } = cached.token1
  const [slot0, liquidity] = await Promise.all([
    client.readContract({ address: poolAddr, abi: POOL_ABI, functionName: 'slot0' }),
    client.readContract({ address: poolAddr, abi: POOL_ABI, functionName: 'liquidity' }),
  ])
  const tick = Number(slot0[1])
  return { tick, price: Math.pow(1.0001, tick) * 10 ** (dec0 - dec1), liquidity: liquidity.toString(), ts: Date.now() }
}

...
import { baseClient, verifyPool, assertStablePair, buildEnterPlan, buildExitTxs, describePlan, poolTick, listPositions, DEFAULTS, CORRIDORS, AERODROME } from './slipstream-core.mjs'
...
export default async function poolsRoutes(app) {
  ...
  // Живой тик пула — лёгкий срез для секундных обновлений фронта.
  app.get('/api/pools/tick/:address', async (req, reply) => {
    try {
      return { ok: true, ...(await poolTick(client, req.params.address)) }
    } catch (e) {
      reply.code(422)
      return { ok: false, error: e.message }
    }
  ...
  // Позиции кошелька в Aerodrome Slipstream (для раздела «Мои позиции»).
  app.get('/api/pools/positions/:wallet', async (req, reply) => {
    try {
      return jsonSafe({ ok: true, ...(await listPositions(client, req.params.wallet)) })
    } catch (e) {
      reply.code(422)
      return { ok: false, error: e.message }
    }

...
import { subscriptionStatus, requireSubscription, preparePermit, buildSubscribeTx, buildCancelTx } from './subscription.mjs'
...
export default async function poolsRoutes(app) {
  ...
  // Подписка: подготовить permit для подписи юзером (только когда контракт задеплоен).
  app.post('/api/pools/subscription/prepare', async (req, reply) => {
    if (!guard(req, reply, rlWrite)) return
    try {
      return jsonSafe({ ok: true, ...(await preparePermit(String(req.body?.wallet || ''))) })
    } catch (e) {
      reply.code(422)
      return { ok: false, error: e.message, code: 'SUB_PREPARE_FAILED' }
    }
  })

  // Подписка: собрать неподписанную tx subscribeWithPermit из permit-подписи юзера.
  app.post('/api/pools/subscription/build', async (req, reply) => {
    if (!guard(req, reply, rlWrite)) return
    try {
      const { value, deadline, signature } = req.body || {}
      return jsonSafe({ ok: true, tx: buildSubscribeTx(value, deadline, signature) })
    } catch (e) {
      reply.code(422)
      return { ok: false, error: e.message, code: 'SUB_BUILD_FAILED' }
    }
  })

  // Подписка: транзакция отмены автопродления.
  app.get('/api/pools/subscription/cancel-tx', async (req, reply) => {
    if (!guard(req, reply, rlRead)) return
    try {
      return jsonSafe({ ok: true, tx: buildCancelTx() })
    } catch (e) {
      reply.code(422)
      return { ok: false, error: e.message, code: 'SUB_CANCEL_FAILED' }
    }
  })
